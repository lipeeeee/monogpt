...
try:
  ...
  if os.path.exists(tokenizer_weigths_path):
    ...
    print(f">>>> Did not find pre-computed tokenizer's weights, training them now on dataset(len={len(dataset)}) with vocab_size={vocab_size}.")
    tokenizer = Tokenizer(models.BPE())
    tokenizer.pre_tokenizer = pre_tokenizers.ByteLevel(add_prefix_space=False)
    trainer = trainers.BpeTrainer(vocab_size=vocab_size, initial_alphabet=[])
    all_files: list = glob.glob("data/*")
